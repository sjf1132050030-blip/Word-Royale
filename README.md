...
| 后端 | Node.js + Express |
| 数据库 | SQLite（`sql.js`，`server/data/word_royale.db`） |
| AI | Gemini Flash（优先）→ Agnes 2.0 Flash（兜底） |

## 快速启动

依赖统一安装在**仓库根目录** `node_modules`（npm workspaces：`client` + `server`）。

```bash
# 配置密钥
cp server/.env.example server/.env
# 编辑 server/.env 填入 GEMINI_API_KEY / AGNES_API_KEY
...
密钥只放在 `server/.env`（已加入 `.gitignore`），**不要写进前端**。
